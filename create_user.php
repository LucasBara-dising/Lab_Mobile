...
echo json_encode(array("mensagem" => "haaa"), JSON_PRETTY_PRINT);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ((isset($data['nome_user']) && isset($data['senha']) && isset($data['email']))) {
        ...
        if (strlen($senha_plana) < 6) {
            echo json_encode(array("mensagem" => "Senha deve ter pelo menos 6 caracteres"), JSON_PRETTY_PRINT);
            exit();
        }
        ...
        $stmt = $conn->prepare("SELECT nome_user FROM TB_Usuario WHERE nome_user = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO TB_Usuario (nome_user, senha, email) VALUES (?, ?, ?)");
